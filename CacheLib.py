...
class LeastRecentlyUsedOrderedDict(OrderedDict):
    # dict for LRUCache(Least Recently Used (LRU) cache implementation.)

    def __init__(self, capacity):
        super(LeastRecentlyUsedOrderedDict, self).__init__()
    ...
    #
    #
    #     pickled_value = OrderedDict.__getitem__


class LRUDictCache(OrderedDict):

    def __init__(self, capacity=250):
        super(LRUDictCache, self).__init__()
        self._capacity = capacity
    ...
    def __setitem__(self, key, value):
        ...
        if containsKey:
            ...
            # print 'add:', (key, value)
        pickled_value = pickle.dumps(value, pickle.HIGHEST_PROTOCOL)
        OrderedDict.__setitem__(self, key, pickled_value)

    def __getitem__(self, key):

        pickled_value = OrderedDict.__getitem__(key)
        try:
